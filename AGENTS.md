...
- **包豪斯折叠吸顶与防叠字纯实色悬浮舱契约 (Collapsible Sticky Header & Anti-Bleed Floating Dock Contract)**：
  ...
  - **预设操作单点归一收拢**：移除场景方案行多余重复的导入按钮，将新建预设、恢复默认与一键导入（`BauhausUiSymbol.Import`）统一收纳在预设胶囊横滑列表末尾；
...
| **Stage 16** | 顶栏折叠吸顶、全场景常驻预设栏与纯实色防叠字悬浮舱 (v1.7.6) | **[x] 已达成** | 彻底根除外部 alpha 模糊库闪退与负 offset 截断 bug；大标题随手势 1:1 动态收缩折叠；预设横滑栏（固定 54.dp）永远稳固吸顶；移除场景方案行重复导入按钮，单点收拢至横滑胶囊列表；顶栏与播控条落地 100% 纯实色防叠字悬浮舱；全站 68 项单测 100% 全绿。 |
